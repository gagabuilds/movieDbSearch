...
export function MovieCard({ movie }: MovieCardProps) {
  ...
  return (
    <Link
      to={`/movie/${movie.id}`}
      className="group relative rounded-lg overflow-hidden bg-card border border-border/50 cursor-pointer transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl hover:shadow-black/60 hover:border-brand/40"
    >
      {/* Poster */}
