...
import { useWatchedList } from '@/hooks/useMovieLists'
...
export function WatchedListPage() {
  const { data, isLoading, isError } = useWatchedList()
  ...
  return (
    <div className="flex min-h-full flex-col p-6">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h1 className="text-xl font-bold tracking-tight">Watched List</h1>
        <span className="text-sm text-muted-foreground">({movies.length})</span>
      </div>

      {isError && (
        <Alert variant="destructive" className="mb-4 max-w-xl">
          <AlertDescription>Failed to load your watched list.</AlertDescription>
        </Alert>
      )}
